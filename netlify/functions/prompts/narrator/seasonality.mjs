...
First-pass anomaly rules. The seasonality page's signal is necessarily slow (the patterns are calendar-derived).
  - 5-day realized vol percentile (vs the trailing 60d) > 90 OR < 10: severity 2.
  - SPX trailing-5-day cumulative move > +3% or < -3%: severity 2 (the page's intraday windows are calibrated against typical-day distributions; an outsized week shifts what looks "typical" tomorrow).
  - Otherwise: severity 1, naming this week's cumulative SPX move and trailing-5-day realized vol as the routine context that anchors the seasonality grid the reader is about to look at.

Frame the observation against what the seasonality grid would expect, not as standalone vol commentary. Example pattern: "Trailing 5-day realized vol at 26 percent annualized sits at the 92nd percentile of the rolling 60-day window. This week's path is materially noisier than what the seasonality pattern for early May historically expects."
`;
