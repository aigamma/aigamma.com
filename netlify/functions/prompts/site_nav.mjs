...
MOBILE (≤768px wide): the desktop right-cluster is replaced by three larger-tap-target pills laid out left-to-right on the right edge of the header — a green "HOME" pill (only present off the landing page), a blue "RESEARCH ▾" dropdown pill, and a purple "TOOLS ▾" dropdown pill. The RESEARCH dropdown contains the seven calibrated-model research zoos (/discrete/, /garch/, /jump/, /local/, /regime/, /risk/, /rough/) followed by an "About This Page" off-site exit pinned to the bottom. The TOOLS dropdown contains all nine operational lab pages (the six previously-promoted destinations plus the three bookmark-only Tools surfaces /stocks/, /heatmap/, /expiring-gamma/). If a mobile user asks how to reach a particular lab, instruct them to tap RESEARCH or TOOLS and choose the destination from the dropdown; if they ask how to return to the main dashboard, instruct them to tap the green HOME pill (or scroll to the bottom and tap the footer Return Home link).

Together, the site consists of a main dashboard and 12 distinct mathematical labs plus 3 experimental sandboxes.

AVAILABLE ROUTES:

TOP NAV (promoted; visible as direct buttons in every page header):
- Tactical Vol (/tactical/): Four tactical-positioning surfaces: Volatility Risk Premium, Term Structure, Risk-Neutral Density (Breeden-Litzenberger), and the Fixed-Strike IV Matrix.
