...
export default function EarningsCalendar() {
  ...
  // Banner inputs — independently sourced so the catalyst banner stays
  // anchored to the Top 100 OV earnings universe regardless of which
  // chart filter the reader has toggled, and pulls macro events from
  // the Forex Factory aggregator at /api/events-calendar in parallel.
  // Both fetches are best-effort and the banner gracefully renders
  ...
  // Banner data — fire once on mount, then re-poll every 10 minutes
  // so the time-to-event countdown stays plausibly fresh through a
  // long page session without hammering either upstream. The Top 100
  // OV earnings response is shared with the chart cache so a
  // toggle-back to topN-100 reads the cached payload instantly.
  useEffect(() => {
    let cancelled = false;
    const ac = new AbortController();
    const loadBannerData = async () => {
      const earningsPromise = (async () => {
        const cached = cacheRef.current.get('topN-100');
        if (cached) return cached;
        try {
          const r = await fetch('/api/earnings?chart_filter=topN-100', { signal: ac.signal });
          if (!r.ok) return null;
          const j = await r.json();
          cacheRef.current.set('topN-100', j);
          return j;
        } catch { return null; }
      })();
      ...
      const [top100, events] = await Promise.all([earningsPromise, eventsPromise]);
      if (cancelled) return;
      if (top100) setTop100Data(top100);
      if (events) setEventsData(events);
    };
    loadBannerData();
    const id = setInterval(loadBannerData, 10 * 60 * 1000);
  ...
  return (
    <div ref={wrapRef} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <CatalystBanner
        calendarDays={top100Data?.calendarDays}
...
// -----------------------------------------------------------------
// Catalyst banner — three time-bucketed pills above the scatter chart
// -----------------------------------------------------------------
//
// Renders three colored pills horizontally between the CalendarHeader
// (the row of legend dots, summary counts, filter toggle, and text-
// size toggle) and the scatter chart. Each pill scopes to a forward
...
//
// Inputs are independently sourced from the chart filter so the
// banner stays anchored to the Top 100 OV earnings universe AND the
// USD macro-event feed regardless of which chart filter the reader
// has toggled. A pill with zero items renders nothing — the banner
// reserves no whitespace for an empty bucket so a quiet 72-hour
// window collapses cleanly.
//
// Catalyst sources:
//
//   1. Earnings — calendarDays from /api/earnings?chart_filter=topN-100.
//      Each ticker is wall-clock-anchored at its session midpoint:
//      BMO at 7:00 AM ET, AMC at 4:30 PM ET, with the historical
//      epsTime field used as a fallback for unsessioned releases.
//      ovRank<=100 is filtered server-side by the topN-100 mode so
//      no per-ticker filtering happens here.
//
//   2. Macro events — events from /api/events-calendar (FF aggregator,
//      USD-only by default). High and Medium impact only — Low and
//      Holiday tiers are filtered out so the banner stays focused on
//      market-moving prints rather than calendar housekeeping.
...
// concentrate in the 6:30-9:00 AM window; AMC releases concentrate in
// the 4:00-5:00 PM window. The midpoint anchor is good enough for the
// 24-hour bucketing the banner does — a release one hour off from its
// anchor only flips a bucket when the actual time straddles a 24-hour
// boundary from "now," and the alternative (per-ticker historical
// epsTime, which is itself the prior quarter's anchor) is no more
// precise for forward releases that haven't been reconfirmed.
...
// regime as the actual ET wall time later in the day. Returns null on
// malformed input.
...
// raw title when no pattern matches.
...
function shortenEventLabel(title) {
  ...
  // No spotlight match — keep the first 3 words, capped at 22 chars.
  const t = String(title).split(/\s+/).slice(0, 3).join(' ');
  return t.length > 22 ? `${t.slice(0, 21)}…` : t;
}

// Format a forward time-to-event as a compact "5h" / "12h" / "45m"
// suffix. Sub-hour offsets render as minutes; otherwise round down to
// the hour so a "23.7h" doesn't visually look like "almost a day"
// (it's actually a hair under one full day to the event).
...
function CatalystBanner({ calendarDays, events, scale }) {
  // 60-second clock tick — the bucket boundaries shift one minute per
  // tick of wall time, but a 24-hour pill doesn't need finer-than-minute
  // resolution on its time-to-event suffix.
    ...
    for (const day of calendarDays || []) {
      for (const t of day.tickers || []) {
        ...
        if (t.sessionLabel === 'BMO') { hour = EARNINGS_BMO_HOUR; minute = EARNINGS_BMO_MINUTE; }
          ...
          // quarter's release time. Extract the H:MM portion and treat
          // it as ET wall-clock (EW's anchor convention).
  ...
  const totalCount = buckets.red.length + buckets.orange.length + buckets.yellow.length;
  const ready = (calendarDays != null) || (events != null);
...
function CatalystPill({ label, title, items, accent, scale }) {
  if (!items || items.length === 0) return null;
  // Cap rendered items so a heavy 100-name day doesn't blow the
  // banner up to 4 lines. 12 visible + a "+N more" tail strikes a
  // balance between a glanceable summary and a complete one — at the
  // typical ovRank<=100 density a 24-hour bucket holds 0-15 names,
  // and the 12 most-imminent are the ones a reader cares about.
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: '999px',
        padding: `${0.5 * scale}rem ${0.95 * scale}rem`,
        display: 'flex',
        flexWrap: 'wrap',
        alignItems: 'center',
        gap: `${0.35 * scale}rem ${0.7 * scale}rem`,
        fontFamily: "Calibri, 'Segoe UI', system-ui, sans-serif",
        fontSize: `${0.95 * scale}rem`,
        color: '#cfd6e6',
        lineHeight: 1.35,
