...
- Names of commercial volatility, options-analytics, gamma-exposure, dealer-positioning, or derivatives-data vendors when used in an attributional, comparative, or stylistic context. This applies to possessive constructions (the vendor's convention), comparative constructions (like the vendor, vendor-inspired), attributional constructions (the vendor's metric, the vendor's approach, the vendor's style), and any related abbreviations used in the same context. Specific vendors covered include but are not limited to Cboe (when used to attribute methodology rather than reference the exchange itself as a market venue), Nations, OptionMetrics, OptionStrat, Unusual Whales, FlowAlgo, Cheddar Flow, BlackBoxStocks, MarketChameleon, and any similar entity in the commercial gamma-exposure, dealer-positioning, or options-analytics space. These vendors may be named only as factual references where strictly necessary, such as documenting that an index is the Cboe VIX or that a quote comes from a specific data API, but never as the source of methodology or convention. If you find yourself writing a commercial vendor's name as the originator of a calculation, the source of a convention, or the comparative reference for how the platform names a metric, stop and rewrite the artifact so the name does not appear in that role at all. The instruction includes the meta-act of documenting a removal: do not name a vendor while describing the elision of that vendor's name.
...
If you are instructed to remove a banned term from any surface, do not document the removal using the banned term. The instruction generalizes to your own output. Specifically: do not write commit messages that say removed all references to X, stripped X from the user-facing surface, scrubbed X from the prose, or any equivalent construction that names the banned term in the act of describing its removal. Beyond that, do not frame the commit as a cleanup operation at all, even when the cleanup phrasing does not name the banned term. Stripped vendor attribution and cleaned up references are still meta-descriptions of a removal action rather than statements of what is now in the code. Phrase the work in terms of what is now present, not the act of removal. Standardized the chart label to Expected Move. Renamed the convention block to its mathematical description. Aligned the gamma exposure prose to its Hull and Natenberg construction. Each of these formulations names the new state of the code, does not re-introduce the banned term, and does not describe the work as a cleanup.

The general principle: if a term is banned in user-facing output, it is banned in developer-facing output describing changes to user-facing output. There is no surface where the banned terms are permitted. The principle extends further: even the meta-act of removal is not the substantive content of the commit. Commit messages name the new state of the code, not the cleanup operation that produced it.

## Commit Message Scope

Commit messages describe what changed in the codebase. They do not describe meta-actions about scrubbing language, cleaning attribution, or removing references. If a commit's only substantive content is the removal of banned terminology, the commit message should name what the new state is, not what the old state was. Example of a wrong commit message: removed vendor references from /earnings chart label. Example of a correct commit message: renamed /earnings chart label to Expected Move. The second formulation is shorter, names the current state of the code, and does not describe the removal.
