...
When the user clicks the extension icon, the popup issues two HTTPS
requests in parallel:

1. `https://aigamma.com/api/snapshot.json` — derived SPX market
   statistics (gamma regime, key positioning levels, volatility metrics,
   prior-day deltas).
2. `https://aigamma.com/api/events-calendar` — public macroeconomic
...
schedule during US equity market hours and updates the toolbar icon to
reflect the current gamma regime. No other network activity occurs.
...
ability to observe or modify traffic on any site other than its two outgoing
fetches to aigamma.com.
...
Both endpoints (`api/snapshot.json` and `api/events-calendar`) are public,
unauthenticated resources. Requests made by the extension are standard HTTPS
...
both endpoints return CORS headers allowing any origin.
