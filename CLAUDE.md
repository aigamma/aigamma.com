...
After step 2, every push to `about.aigamma.com/main` that touches `index.html` triggers a refresh-rag run on this repo within minutes. Verify by pushing an inert about-page edit and watching the Actions tab here for a run with `event_name: repository_dispatch` and `client_payload.source_repo: aigamma/about.aigamma.com`.
