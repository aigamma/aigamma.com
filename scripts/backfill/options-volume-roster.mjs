...
// Slim derived output — top-50 anchor list with key fields only,
// sorted by ascending joint-rank harmonic-mean score (so the
// best-aligned NVDA-class names come first). Written alongside the
// full OV roster on every run. Future surfaces that only need the
// durable-core list (homepage tiles, summary cards, extension
// popup, the dashboard's rumored "anchored watchlist") can consume
// this slim file without parsing the full 250-name roster — saves
// ~95 KB of disk on cold-start and keeps the consumer surface
// area decoupled from the full roster's internal field churn.
const DEFAULT_ANCHOR_OUT = 'src/data/anchor-50.json';
...
function parseArgs(argv) {
  const out = {
    csvPath: null,
    sheetsDir: DEFAULT_SHEETS_DIR,
    outPath: DEFAULT_OUT,
    anchorOutPath: DEFAULT_ANCHOR_OUT,
  };
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === '--anchor-out') out.anchorOutPath = argv[++i];
  }
  return out;
}
...
async function main() {
  ...
  // Slim Anchor 50 output. Re-derive the joint-rank score per anchor
  // (cheap — 50 names, three multiplications each) so the slim file
  // is self-sorted in score-ascending order, without dragging score
  // computation into the per-holding objects on the full roster
  // (the full roster's holdings array stays sorted by ovRank desc
  // for backward-compat with consumers that read it that way).
  // Each entry keeps only the seven fields a downstream surface
  // typically needs: symbol, name, sector, weight, ovRank, mcRank,
  // hype. The full-roster optionsVolume column is dropped because
  // the absolute volume number is rarely needed once anchor
  // membership is established — the rank IS the consumer-facing
  // signal.
  const anchorHoldings = holdings
    ...
    .map((h) => ({
      ...
      mcRank: h.mcRank,
      hype: h.hype,
      score: +((2 * h.ovRank * h.mcRank) / (h.ovRank + h.mcRank)).toFixed(3),
    }))
    .sort((a, b) => a.score - b.score);
  const anchorPayload = {
    generatedAt: new Date().toISOString(),
    sourceCsv: csvPath,
    sp500AsOf,
    count: anchorHoldings.length,
    description:
      'Top-N OV ∩ SP500 names by ascending harmonic mean of (ovRank, mcRank). ' +
      'Derived from src/data/options-volume-roster.json. Refresh by running ' +
      'scripts/backfill/options-volume-roster.mjs.',
    holdings: anchorHoldings,
  };
  mkdirSync(dirname(args.anchorOutPath), { recursive: true });
  writeFileSync(
    args.anchorOutPath,
