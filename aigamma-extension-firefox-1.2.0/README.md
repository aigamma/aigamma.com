...
2. Click Load Temporary Add-on and select any file inside the
   `aigamma-extension-firefox-1.2.0` folder (Firefox loads the entire
   directory).
...
4. Zip the *contents* of this folder (not the folder itself). On Windows
   PowerShell, from inside `aigamma-extension-firefox-1.2.0/`:

        Compress-Archive -Path * -DestinationPath ..\aigamma-extension-firefox-1.2.0.zip

   The repo root also contains a pre-built
   `aigamma-extension-firefox-1.2.0.zip` ready for upload.
...
The Chrome build at `../aigamma-extension-1.2.0/` is the same code
without the `browser_specific_settings.gecko` block. Submit
`../aigamma-extension-1.2.0.zip` (also pre-built at the repo root) to
